...
double dy0(int i)
{
	...
	double y0 = stod(file[49][C]), y1 = stod(file[50][C]), y2 = stod(file[51][C]), y3 = stod(file[52][C]), y4 = stod(file[53][C]), y5 = stod(file[54][C]);
...
double p(int i, double x)
{
	...
	double x0 = stod(file[49][0]), x1 = stod(file[50][0]), x2 = stod(file[51][0]), x3 = stod(file[52][0]), x4 = stod(file[53][0]), x5 = stod(file[54][0]);
	...
	case 2:
		return (x - x5)*(x - x4);
	case 3:
		return (x - x5)*(x - x4)*(x - x3);
	case 4:
		return (x - x5)*(x - x4)*(x - x3)*(x - x2);
	case 5:
		return (x - x5)*(x - x4)*(x - x3)*(x - x2)*(x - x1);
...
int main() {
	...
	for (int k = 49; k < 55; k++)
	{
		...
		{
			fout << (stod(file[k][0]) + l) << "    " << newton2((stod(file[k][0]) + l)) << endl;
		}
	}
	for (int k = 49; k < 55; k++)
	{
